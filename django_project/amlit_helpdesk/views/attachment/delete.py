...
@login_required
def attachment_delete(request, ticket_id, followup_id, attachment_id):
    if request.method == 'DELETE':
        ticket = get_object_or_404(Ticket, id=ticket_id)
        followup = get_object_or_404(FollowUp, id=followup_id)
        attachment = get_object_or_404(FollowUpAttachment, id=attachment_id)
        if can_delete_follow_up(ticket, followup, request.user) and attachment.followup == followup:
            attachment.delete()
